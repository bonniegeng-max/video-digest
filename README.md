...
A bare YouTube URL is enough. The phrase “视频深读” is optional.
...
python3 scripts/doctor.py --network
...
```text
<system-temp>/video-deep-reader/<channel>/<video-id>/
```

This is a cache, not durable storage. A note is saved elsewhere only when the user explicitly requests a file.
...
Notes follow the language of the current request. Script messages default to English; pass `--ui-lang zh` for Chinese.
